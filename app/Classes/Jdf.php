...
class Jdf{
    public static function jdate($format,$timestamp='',$none='',$time_zone='Asia/Tehran',$tr_num='fa'){
        ...
        $ts=$T_sec+(($timestamp==='')?time():Jdf::tr_num($timestamp));
        ...
        for($i=0; $i<$sl; $i++){
            ...
            switch($sub){
                ...
                case'v':
                    $out.=Jdf::jdate_words(array('ss'=>($j_y%100)),' ');
                    break;
    ...
    public static function jmktime($h='',$m='',$s='',$jm='',$jd='',$jy='',$none='',$timezone='Asia/Tehran'){
        ...
        if($h===''){
            ...
            list($h,$m,$s,$jm,$jd,$jy)=explode('_',Jdf::tr_num($h.'_'.$m.'_'.$s.'_'.$jm.'_'.$jd.'_'.$jy));
            if($m===''){
                return mktime($h);
            }else{
                if($s===''){
    ...
    public static function jcheckdate($jm,$jd,$jy){
        list($jm,$jd,$jy)=explode('_',Jdf::tr_num($jm.'_'.$jd.'_'.$jy));
    ...
    public static function gregorian_to_jalali($gy,$gm,$gd,$mod=''){
        list($gy,$gm,$gd)=explode('_',Jdf::tr_num($gy.'_'.$gm.'_'.$gd));/* <= Extra :اين سطر ، جزء تابع اصلي نيست */
    ...
    public static function  jalali_to_gregorian($jy,$jm,$jd,$mod=''){
        list($jy,$jm,$jd)=explode('_',Jdf::tr_num($jy.'_'.$jm.'_'.$jd));/* <= Extra :اين سطر ، جزء تابع اصلي نيست */
        if($jy > 979){
            $gy=1600;
            $jy-=979;
        }else{
