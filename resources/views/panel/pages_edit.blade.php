    ...
    <div class="col-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                ...
                <form class="forms-sample" method="post" action="{{ url('/dashboard/pages/edite/'.$data['page']->id) }}">
                    @csrf
                    <div class="form-group">
                        <label for="title">Title</label>
                        <input type="text" name="title" class="form-control" id="title" placeholder="Page Title" value="{{ old('title') ? old('title') : $data['page']->title }}">
                    </div>
                    <div class="form-group">
                        <label for="description">Description</label>
                        <input type="text" class="form-control" name="description" id="description" placeholder="Page Description" value="{{ old('description') ? old('description') : $data['page']->description }}">
                    </div>
                    <div class="form-group">
                        <label>Image upload</label>
                        <div class="input-group col-xs-12">
                            <input type="text" name="image" class="form-control" id="selected-image" placeholder="Image URL" value="{{ old('image') ? old('image') : $data['page']->image }}">
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" name="body" id="body" rows="4">{{ old('body') ? old('body') : $data['page']->body }}</textarea>
                    </div>
                    <div class="form-group">
                        <label for="title">Language</label>
                        <select class="form-control" id="language" name="language">
                            @foreach($data['lan'] as $lan)
                                <option value="{{ $lan->id }}" <?php if (old('language') == $lan->id or $data['page']->language_id == $lan->id){echo 'selected';} ?>>{{ $lan->name }}</option>
                            @endforeach
                        </select>
                    </div>
